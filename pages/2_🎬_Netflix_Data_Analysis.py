...
num_missing_directors = movies_df[movies_df['director'].isna()].shape[0]

# TODO: Ex 2.4: How many different countries are there in the data?
country_set = set(list(x.strip() for x in','.join(movies_df[movies_df['country'].notna()]['country']).split(',') if x))
n_countries = len(country_set)

# TODO: Ex 2.5: How many characters long are on average the title names?
avg_title_length = round(movies_df['title'].apply(lambda x: len(x)).mean(),0)
...
for c in country_set:
    top_countries[c] = movies_df[(movies_df['country'].fillna('Unknown').str.contains(c)) & (movies_df['release_year']==year)].shape[0]

top_10_countries = pd.Series(top_countries).nlargest(10)
...
temp_df_duration = movies_df[movies_df['type']=='Movie']
temp_df_duration['duration'] = temp_df_duration['duration'].apply(lambda x: round(int(x[:-4]),2))
movies_avg_duration_per_year = temp_df_duration.groupby('release_year')['duration'].mean()
